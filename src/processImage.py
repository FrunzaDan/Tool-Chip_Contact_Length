...
def process_this_image_function(current_image_path, current_image_name) -> None:
    if current_image_path:
        ...
        try:
            current_resized_image: cv2.typing.MatLike = cv2.resize(current_image, (1080, int(
                int(current_image.shape[0])*1080/int(current_image.shape[1]))), cv2.INTER_LINEAR)
            current_resize_image_plot: cv2.typing.MatLike = current_resized_image.copy()
        except cv2.error as resize_error:
            print("Resize Error at " + current_image_name +
                  ": " + str(resize_error))
        # Crop the Canny detected image in the right half:
        try:
            current_cropped_image_plot: cv2.typing.MatLike = current_resized_image[0:int(current_resized_image.shape[0]), int(
        ...
        # grayscale the image:
        try:
            grayscaled_image: cv2.typing.MatLike = cv2.cvtColor(
        ...
        try:
            ...
            morph_closed_image: cv2.typing.MatLike = cv2.morphologyEx(
        ...
        try:
            ...
            dilated_image: cv2.typing.MatLike = cv2.dilate(
        ...
        try:
            canny_edges_image: cv2.typing.MatLike = getContours.get_Contours_Function(
        ...
        # HoughP Transform:
        try:
            hough_image_plot: cv2.typing.MatLike = getHoughLines.get_hough_lines_function(
