...
def get_Contours_Function(dilation_img, threshold_1, threshold_2, kernel_size):
    if ((dilation_img is None)):
        ...
        try:
            blurred_image = cv2.GaussianBlur(dilation_img, (9, 9), 1)
        except cv2.error as blur_exception:
            print(blur_exception)
        try:
            canny_image = cv2.Canny(blurred_image, threshold_1,
